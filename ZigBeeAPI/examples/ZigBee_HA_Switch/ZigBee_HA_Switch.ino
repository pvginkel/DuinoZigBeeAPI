...
//***************************************
  ...
  const byte buttonPin = 7;          // This is the pin that the button is attached to that will locally turn the
                                     // LED on and off. Connect the button between GND and this pin
...
void SendEnumerationReport(boolean Value)
{
  ...
  zb.TX_Indirect(0x38, 0x0104, 0x0006, Buffer, 7);                            // TX_Indirect(sEP, Prfl, Clstr, BuffAdd, BuffSize)
}
...
void loop()
{
  ...
  do                                                                          // Begin polling the button and checking for incoming packets
  {
  	...
  	if (digitalRead(buttonPin) == true) {buttonReleased = true;};             // Check if the button has been released
